...
class SAGAN(object):
    ...
    def testarithmetic(self):
        ...
        for i in range(self.test_num) :
            z_sample_a = np.random.uniform(-1, 1, size=(self.batch_size, 1, 1, self.z_dim))
            z_sample_b = np.random.uniform(-1, 1, size=(self.batch_size, 1, 1, self.z_dim))
            z_sample_c = np.random.uniform(-1, 1, size=(self.batch_size, 1, 1, self.z_dim))
            z_sample_d = z_sample_a - z_sample_b + z_sample_c

            samples_a = self.sess.run(self.fake_images, feed_dict={self.z: z_sample_a})
            save_images(samples_a[:image_frame_dim * image_frame_dim, :, :, :],
                        [image_frame_dim, image_frame_dim],
                        result_dir + '/' + self.model_name + '_test_a_{}.png'.format(i))

            samples_b = self.sess.run(self.fake_images, feed_dict={self.z: z_sample_b})
            save_images(samples_b[:image_frame_dim * image_frame_dim, :, :, :],
                        [image_frame_dim, image_frame_dim],
                        result_dir + '/' + self.model_name + '_test_b_{}.png'.format(i))

            samples_c = self.sess.run(self.fake_images, feed_dict={self.z: z_sample_c})
            save_images(samples_c[:image_frame_dim * image_frame_dim, :, :, :],
                        [image_frame_dim, image_frame_dim],
                        result_dir + '/' + self.model_name + '_test_c_{}.png'.format(i))

            samples_d = self.sess.run(self.fake_images, feed_dict={self.z: z_sample_d})
            save_images(samples_d[:image_frame_dim * image_frame_dim, :, :, :],
                        [image_frame_dim, image_frame_dim],
                        result_dir + '/' + self.model_name + '_test_d_{}.png'.format(i))
